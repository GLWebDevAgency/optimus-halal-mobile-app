# Conditions Generales d'Utilisation — Naqiy

**Derniere mise a jour : 7 mars 2026**
**Version : 1.0**

---

## 1. Objet

Les presentes Conditions Generales d'Utilisation (ci-apres "CGU") ont pour objet de definir les modalites et conditions dans lesquelles **GL Web Dev Agency** (ci-apres "l'Editeur") met a disposition l'application mobile **Naqiy** (ci-apres "l'Application") et les services associes (ci-apres "les Services") aux utilisateurs (ci-apres "l'Utilisateur" ou "vous").

L'utilisation de l'Application implique l'acceptation pleine et entiere des presentes CGU. Si vous n'acceptez pas ces conditions, vous ne devez pas utiliser l'Application.

---

## 2. Mentions legales

**Editeur** : GL Web Dev Agency
**Forme juridique** : [A completer]
**SIRET** : [A completer]
**Siege social** : [Adresse a completer]
**Directeur de la publication** : [Nom a completer]
**Email** : contact@naqiy.com
**Hebergement** : Railway Corp., 548 Market St, San Francisco, CA 94104, USA

---

## 3. Description des Services

### 3.1. Presentation generale

Naqiy est une application mobile d'aide a la consommation halal. Elle propose les fonctionnalites suivantes :

- **Scan de produits** : identification de produits alimentaires par code-barres et analyse de leur conformite halal (ingredients, additifs, certification)
- **Verdicts halal** : attribution d'un statut halal (halal, douteux, haram) fonde sur des sources religieuses referencees et des bases de donnees ouvertes
- **Carte des commerces** : localisation de magasins, boucheries et restaurants certifies halal a proximite
- **Alertes ethiques** : notifications relatives aux rappels de produits, fraudes a la certification, informations communautaires
- **Articles** : contenu editorial et educatif sur la consommation halal
- **Profil et preferences** : personnalisation selon l'ecole juridique (madhab), les allergenes et les restrictions alimentaires

### 3.2. Nature informative des verdicts

**Les verdicts halal fournis par l'Application ont un caractere strictement informatif et ne constituent en aucun cas une certification halal officielle, un avis religieux (fatwa) ou un conseil dietetique ou medical.**

Les analyses sont basees sur :
- Les donnees publiques d'OpenFoodFacts (base contributive)
- Les listes d'additifs et ingredients issus de sources religieuses referencees
- Les informations declarees par les fabricants
- Les contributions de la communaute

L'Editeur ne garantit pas l'exhaustivite, l'exactitude ou l'actualite de ces informations. L'Utilisateur est invite a exercer son propre discernement et a consulter un savant religieux qualifie (alim) pour toute question de jurisprudence islamique.

### 3.3. Dependance aux donnees tierces

L'Application s'appuie sur des sources de donnees externes (OpenFoodFacts, Google Places, organismes de certification). L'Editeur n'est pas responsable :
- De l'indisponibilite temporaire de ces sources
- Des erreurs ou omissions dans les donnees fournies par ces tiers
- De la modification unilaterale de ces donnees par leurs fournisseurs

---

## 4. Conditions d'acces

### 4.1. Acces en mode invite (anonyme)

L'Application permet un acces limite sans creation de compte (mode invite). Dans ce mode :
- Le nombre de scans quotidiens est limite (quota journalier)
- L'historique des scans n'est pas sauvegarde
- Les fonctionnalites de favoris, alertes personnalisees et preferences sont indisponibles
- La geolocalisation est utilisee en temps reel uniquement, sans conservation

### 4.2. Acces avec compte

La creation d'un compte gratuit permet l'acces a l'ensemble des fonctionnalites de base :
- Scans illimites (offre gratuite) ou etendus (abonnement Naqiy+)
- Historique des scans synchronise
- Favoris et dossiers de favoris
- Alertes ethiques personnalisees
- Preferences alimentaires et religieuses

### 4.3. Conditions d'inscription

Pour creer un compte, l'Utilisateur doit :
- Etre age d'au moins **16 ans**
- Fournir une adresse email valide
- Accepter les presentes CGU et la Politique de Confidentialite
- Definir un mot de passe securise

L'Utilisateur s'engage a fournir des informations exactes et a les maintenir a jour. Tout compte cree avec des informations manifestement fausses pourra etre suspendu ou supprime.

### 4.4. Securite du compte

L'Utilisateur est seul responsable de la confidentialite de ses identifiants de connexion. Toute utilisation de l'Application effectuee depuis son compte est presumee realisee par lui. En cas d'utilisation non autorisee de son compte, l'Utilisateur doit en informer l'Editeur immediatement a l'adresse contact@naqiy.com.

---

## 5. Abonnement Naqiy+

### 5.1. Description

L'Application propose un abonnement premium denomme **Naqiy+** donnant acces a des fonctionnalites avancees, notamment :
- Scans illimites sans publicite
- Analyses detaillees des ingredients
- Acces prioritaire aux nouvelles fonctionnalites
- Support prioritaire

Le contenu exact de l'offre Naqiy+ est decrit sur la page d'abonnement de l'Application et peut evoluer. Toute modification substantielle sera communiquee aux abonnes existants.

### 5.2. Tarification et paiement

- Les prix sont affiches dans l'Application en euros (EUR), TTC, et peuvent varier selon le pays de l'Utilisateur
- Le paiement est gere exclusivement par les plateformes **Apple App Store** (Apple Inc.) et **Google Play Store** (Google LLC)
- L'Editeur n'a jamais acces aux informations bancaires ou de paiement de l'Utilisateur
- Les conditions de paiement, facturation et remboursement sont regies par les conditions generales d'Apple ou de Google, selon la plateforme utilisee

### 5.3. Renouvellement et resiliation

- L'abonnement est reconduit **tacitement** a chaque periode (mensuelle ou annuelle, selon l'offre souscrite)
- L'Utilisateur peut resilier son abonnement a tout moment depuis les parametres de son compte Apple ou Google
- La resiliation prend effet a la fin de la periode en cours ; l'Utilisateur conserve l'acces aux fonctionnalites premium jusqu'a cette date
- Aucun remboursement au prorata n'est effectue pour la periode entamee, conformement aux politiques des plateformes Apple et Google

### 5.4. Droit de retractation

Conformement a l'article L.221-28 du Code de la consommation, le droit de retractation ne s'applique pas aux contenus numeriques fournis sur un support immateriel dont l'execution a commence avec l'accord du consommateur. En souscrivant a Naqiy+, l'Utilisateur accepte que l'acces aux fonctionnalites premium soit immediat et renonce expressement a son droit de retractation.

Toutefois, les politiques de remboursement d'Apple et de Google peuvent prevoir des exceptions. L'Utilisateur est invite a consulter les conditions de la plateforme concernee.

---

## 6. Obligations de l'Utilisateur

### 6.1. Utilisation licite

L'Utilisateur s'engage a utiliser l'Application conformement aux presentes CGU, aux lois et reglements en vigueur, et dans le respect des droits des tiers. Il s'interdit notamment de :

- Utiliser l'Application a des fins illicites, frauduleuses ou contraires a l'ordre public
- Tenter d'acceder a des fonctionnalites, donnees ou zones de l'Application auxquelles il n'a pas droit
- Contourner, desactiver ou alterer les mesures de securite, de limitation d'acces ou de quota
- Proceder a du reverse engineering, de la decompilation ou du desassemblage de l'Application
- Utiliser des robots, scrapers ou tout systeme automatise pour acceder a l'Application ou en extraire des donnees
- Surcharger deliberement les serveurs de l'Application (attaques par deni de service)

### 6.2. Contributions communautaires

Lorsque l'Utilisateur publie un avis, une note ou un signalement (ci-apres "Contributions"), il s'engage a ce que :

- Le contenu soit exact, honnete et fonde sur une experience reelle
- Le contenu ne soit pas diffamatoire, injurieux, discriminatoire ou contraire aux bonnes moeurs
- Le contenu ne contienne pas de donnees personnelles de tiers sans leur consentement
- Le contenu ne constitue pas de la publicite deguisee ou du spam

L'Editeur se reserve le droit de supprimer, sans preavis ni indemnite, toute Contribution ne respectant pas ces conditions.

### 6.3. Licence sur les Contributions

En publiant une Contribution, l'Utilisateur accorde a l'Editeur une licence non exclusive, mondiale, gratuite, sous-licenciable et cessible, pour la duree des droits de propriete intellectuelle applicables, permettant d'utiliser, reproduire, modifier, adapter, publier et afficher ladite Contribution dans le cadre du fonctionnement et de la promotion de l'Application.

L'Utilisateur conserve l'integralite de ses droits de propriete intellectuelle sur ses Contributions et peut les supprimer a tout moment depuis l'Application.

---

## 7. Propriete intellectuelle

### 7.1. Droits de l'Editeur

L'Application, son architecture, son code source, ses interfaces, son design, ses logos, ses textes, ses algorithmes d'analyse et l'ensemble de ses elements constitutifs sont proteges par le droit d'auteur, le droit des marques et le droit des bases de donnees.

La marque **Naqiy** et le logo associe sont la propriete exclusive de GL Web Dev Agency. Toute reproduction, representation ou utilisation non autorisee est interdite.

### 7.2. Licence d'utilisation

L'Editeur concede a l'Utilisateur un droit d'utilisation personnel, non exclusif, non cessible, non sous-licenciable et revocable de l'Application, pour la duree de son inscription et dans le cadre d'un usage strictement prive et non commercial.

### 7.3. Donnees ouvertes

L'Application utilise des donnees issues d'**OpenFoodFacts**, mises a disposition sous licence Open Database License (ODbL). Ces donnees restent soumises aux conditions de cette licence.

---

## 8. Responsabilite de l'Editeur

### 8.1. Obligation de moyens

L'Editeur s'engage a mettre en oeuvre les moyens necessaires pour assurer le bon fonctionnement et la securite de l'Application. Cette obligation est une obligation de **moyens** et non de resultat.

### 8.2. Limitation de responsabilite

**L'Editeur ne saurait etre tenu responsable :**

- Des decisions prises par l'Utilisateur sur la base des informations fournies par l'Application (verdicts halal, analyse d'ingredients, statut de certification)
- Des dommages directs ou indirects resultant de l'utilisation ou de l'impossibilite d'utiliser l'Application
- Des erreurs, inexactitudes ou omissions dans les donnees affichees, qu'elles proviennent de l'Editeur ou de sources tierces
- Des interruptions de service, qu'elles soient planifiees (maintenance) ou imprevues (panne technique, force majeure)
- Des actes de tiers (piratage, attaques informatiques) malgre la mise en oeuvre de mesures de securite raisonnables
- De la perte de donnees en cas de suppression de compte demandee par l'Utilisateur
- Des consequences de la revocation, suspension ou modification d'une certification halal par un organisme tiers

### 8.3. Exclusion de garantie religieuse

**L'Application n'est affiliee a aucun organisme religieux, conseil de la fatwa, mosquee, federation islamique ou autorite religieuse.** Les verdicts et analyses proposes sont le resultat d'un traitement algorithmique fonde sur des donnees publiques et des sources documentaires.

L'Editeur ne fournit aucune garantie quant a la conformite religieuse absolue des verdicts. L'Utilisateur reconnait que la qualification halal d'un produit peut varier selon les ecoles juridiques et les interpretations savantes.

### 8.4. Force majeure

L'Editeur ne saurait etre tenu responsable de l'inexecution de ses obligations en cas de force majeure au sens de l'article 1218 du Code civil, notamment : catastrophe naturelle, epidemie, guerre, greve, defaillance des reseaux de telecommunications, interruption de service des fournisseurs d'hebergement, decision administrative ou judiciaire.

---

## 9. Disponibilite du service

### 9.1. Acces au service

L'Editeur s'efforce de maintenir l'Application accessible 24 heures sur 24, 7 jours sur 7. Toutefois, l'acces peut etre interrompu temporairement pour :

- Maintenance technique (programmee ou d'urgence)
- Mises a jour de l'Application
- Cas de force majeure
- Dysfonctionnement technique independant de la volonte de l'Editeur

### 9.2. Modifications du service

L'Editeur se reserve le droit de faire evoluer, modifier ou supprimer tout ou partie des fonctionnalites de l'Application, a tout moment et sans preavis pour les fonctionnalites gratuites.

Pour les fonctionnalites incluses dans l'abonnement Naqiy+, toute suppression substantielle sera notifiee aux abonnes avec un preavis raisonnable de **30 jours**.

---

## 10. Donnees personnelles

Le traitement des donnees personnelles de l'Utilisateur est regi par notre **Politique de Confidentialite**, accessible dans l'Application et a l'adresse [politique de confidentialite]. Cette Politique de Confidentialite fait partie integrante des presentes CGU.

Pour toute question relative a vos donnees personnelles, contactez notre Delegue a la Protection des Donnees : dpo@naqiy.com.

---

## 11. Signalement et moderation

### 11.1. Signalement de contenu

L'Utilisateur peut signaler tout contenu qu'il estime contraire aux presentes CGU ou a la loi, notamment :
- Un avis frauduleux ou diffamatoire sur un magasin
- Un produit dont les informations sont manifestement erronees
- Un bug ou dysfonctionnement de l'Application

Les signalements sont traites dans un delai raisonnable. L'Editeur n'est pas tenu de communiquer les suites donnees a chaque signalement.

### 11.2. Moderation des avis

Les avis et notes publies par les Utilisateurs font l'objet d'une moderation a posteriori. L'Editeur se reserve le droit de supprimer tout avis qui :
- Est manifestement faux ou trompeur
- Contient des propos injurieux, discriminatoires ou hors sujet
- Constitue du spam ou de la publicite
- Viole les droits de tiers

---

## 12. Suspension et resiliation

### 12.1. Suspension par l'Editeur

L'Editeur se reserve le droit de suspendre temporairement ou definitivement l'acces d'un Utilisateur a l'Application en cas de :

- Violation des presentes CGU
- Utilisation abusive ou frauduleuse du service
- Comportement nuisible a la communaute ou au fonctionnement de l'Application
- Demande d'une autorite administrative ou judiciaire

L'Utilisateur sera informe de la suspension et de ses motifs par email, sauf en cas d'urgence ou d'obligation legale contraire.

### 12.2. Suppression de compte par l'Utilisateur

L'Utilisateur peut supprimer son compte a tout moment depuis les parametres de l'Application (Parametres > Supprimer mon compte). La suppression entraine :

- L'effacement de ses donnees personnelles dans un delai de **30 jours**
- L'anonymisation de ses Contributions (avis, notes) sauf demande explicite de suppression
- La perte de l'acces a l'historique de ses scans et favoris
- La resiliation de fait de l'abonnement Naqiy+ (sans remboursement de la periode en cours)

### 12.3. Consequences de la resiliation

Apres suppression du compte, l'Utilisateur perd l'acces a toutes les fonctionnalites reservees aux membres. Les clauses des presentes CGU relatives a la propriete intellectuelle, la limitation de responsabilite et la loi applicable survivent a la resiliation.

---

## 13. Liens hypertextes

L'Application peut contenir des liens vers des sites ou services tiers (sources d'articles, sites de certification, liens de partage). L'Editeur n'exerce aucun controle sur ces sites et n'engage pas sa responsabilite quant a leur contenu, leur politique de confidentialite ou leurs pratiques.

---

## 14. Notifications push

L'Application peut envoyer des notifications push avec le consentement de l'Utilisateur. Ces notifications peuvent concerner :

- Des alertes ethiques (rappels de produits, fraudes)
- Des informations techniques (mise a jour de l'application)
- Des communications liees au compte (expiration d'abonnement)

L'Utilisateur peut desactiver les notifications a tout moment depuis les parametres de son appareil ou de l'Application.

---

## 15. Droit applicable et juridiction

### 15.1. Droit applicable

Les presentes CGU sont regies par le **droit francais**.

### 15.2. Reglement amiable

En cas de litige relatif a l'interpretation ou a l'execution des presentes CGU, les parties s'engagent a rechercher une solution amiable avant toute action judiciaire.

Conformement aux articles L.611-1 et suivants du Code de la consommation, l'Utilisateur peut recourir gratuitement a un mediateur de la consommation en vue de la resolution amiable du litige. Le mediateur competent est :

**[Nom du mediateur a completer]**
Site web : [A completer]

L'Utilisateur peut egalement deposer une reclamation sur la plateforme europeenne de reglement en ligne des litiges : [https://ec.europa.eu/consumers/odr](https://ec.europa.eu/consumers/odr)

### 15.3. Juridiction competente

A defaut de resolution amiable, tout litige sera soumis aux tribunaux competents du ressort du siege social de l'Editeur, sauf disposition legale imperative contraire (notamment la competence du tribunal du domicile du consommateur).

---

## 16. Modification des CGU

L'Editeur se reserve le droit de modifier les presentes CGU a tout moment. Les modifications entrent en vigueur des leur publication dans l'Application.

En cas de modification substantielle :
- L'Utilisateur sera informe par notification dans l'Application ou par email
- Un delai raisonnable de **15 jours** sera accorde avant l'entree en vigueur des nouvelles conditions
- L'Utilisateur pourra refuser les nouvelles conditions en supprimant son compte avant leur entree en vigueur

La poursuite de l'utilisation de l'Application apres l'entree en vigueur des modifications vaut acceptation des CGU modifiees.

---

## 17. Divisibilite

Si l'une quelconque des stipulations des presentes CGU est declaree nulle ou inapplicable par une juridiction competente, cette nullite n'affectera pas la validite des autres stipulations, qui demeureront en vigueur.

---

## 18. Integralite

Les presentes CGU, la Politique de Confidentialite et, le cas echeant, les conditions de l'abonnement Naqiy+ constituent l'integralite de l'accord entre l'Utilisateur et l'Editeur relatif a l'utilisation de l'Application. Elles se substituent a toute version anterieure.

---

## 19. Contact

Pour toute question relative aux presentes Conditions Generales d'Utilisation :

- **Email** : contact@naqiy.com
- **Objet** : [CGU] Votre demande

---

*Document redige conformement au Code civil, au Code de la consommation, a la Loi pour la Confiance dans l'Economie Numerique (LCEN) n°2004-575, au RGPD (Reglement UE 2016/679), aux exigences de l'Apple App Store (Guidelines 3.1.1, 3.1.2, 5.1.1) et du Google Play Store (Developer Distribution Agreement, User Data policy).*

*Naqiy — "Scanne. Comprends. Choisis."*
